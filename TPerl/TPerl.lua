...
local function TPerl_SetAuraStackText(button, legacyCount, unit, auraInstanceID)
	...
	if (self.conf and UnitExists(partyid)) then
		...
		if (self.conf.buffs.enable and maxBuffs and maxBuffs > 0) then
			...
			if (C_UnitAuras and C_UnitAuras.GetAuraDataByIndex) then
				playerBuffByID = { }
				for i = 1, maxBuffs do
					local aura = C_UnitAuras.GetAuraDataByIndex(partyid, i, "HELPFUL|PLAYER")
					if (not aura) then
						break
					end
			...
			for mine = 1, 2 do
				...
				for buffnum = 1, maxBuffs do
					...
					if (not name) then
						...
						if (C_UnitAuras and C_UnitAuras.GetAuraDataByIndex) then
							local aura = C_UnitAuras.GetAuraDataByIndex(partyid, buffnum, filter)
		...
		if (canCooldown) then
			...
			if (duration and expirationTime and TPerl_CanAccess(duration) and TPerl_CanAccess(expirationTime) and duration > 0 and expirationTime > 0) then
				...
				if (auraInstanceID and C_UnitAuras and C_UnitAuras.GetAuraDuration and button.cooldown.SetCooldownFromDurationObject) then
					local durObj = C_UnitAuras.GetAuraDuration(partyid, auraInstanceID)
					if (durObj) then
		...
		if (self.conf.debuffs.enable and maxDebuffs and maxDebuffs > 0) then
			...
			if (C_UnitAuras and C_UnitAuras.GetAuraDataByIndex) then
				...
				for i = 1, maxDebuffs do
					local aura = C_UnitAuras.GetAuraDataByIndex(partyid, i, "HARMFUL|PLAYER")
					if (not aura) then
						break
					end
					local aid = aura.auraInstanceID
...
if (C_UnitAuras and C_UnitAuras.GetAuraDataByIndex) then
	local aura = C_UnitAuras.GetAuraDataByIndex(partyid, auraIndex or buffnum, filter)
	auraInstanceID = aura and aura.auraInstanceID
	if (playerDebuffByID and auraInstanceID and playerDebuffByID[auraInstanceID]) then
		isPlayer = true
					...
					if (icon and (((mine == 1) and isPlayer) or ((mine == 2) and not isPlayer))) then
						...
						if (TPerl_CanAccess(debuffType)) then
							debuffKey = debuffType
						end
						local borderColor = (DebuffTypeColor and DebuffTypeColor[debuffKey])
						borderColor = borderColor or (DebuffTypeColor and (DebuffTypeColor.none or DebuffTypeColor["none"]))
						if (borderColor) then
