...
local function TPerl_AB_SafeBool(v)
	...
	-- Only boolean-test if the value is accessible.
...
local function TPerl_AB_SpellNameFromID(spellID)
	...
	if C_Spell_GetSpellName then
		local n = C_Spell_GetSpellName(spellID)
		if n then
			return n
		end
	end
...
local function TPerl_AB_GetCastText(name, text, spellID)
	if name and TPerl_AB_CanAccess(name) then
		return TPerl_AB_StripNoText(name)
	end
	if text and TPerl_AB_CanAccess(text) then
		return TPerl_AB_StripNoText(text)
	end
	local n = TPerl_AB_SpellNameFromID(spellID)
	if n then
		return TPerl_AB_StripNoText(n)
	end
...
local function ActiveCasting(self)
	...
	if (name) then
		...
		if (canaccessvalue and endTime and not canaccessvalue(endTime)) then
			return true
		end
		if (endTime and endTime > t + 500) then
			return true
		end
	end
end
...
function TPerl_ArcaneBar_OnEvent(self, event, unit, ...)
	...
	if (event == "UNIT_SPELLCAST_START") then
		...
		local displayName = TPerl_AB_GetCastText(name, text, spellID)
		...
		self.barSpark:Show()
		if (not startTime or not endTime or not TPerl_AB_CanAccess(startTime) or not TPerl_AB_CanAccess(endTime)) then
			self:Hide()
			return
		end
